...
const BatchProcessingUI = ({
  ...
  const handleFileProcessed = (data) => {
    ...
    //Definir funcion de procesamiento
  ...
  //Definir funcion de cierre
  const closeModal = () => {
    setModal({ ...modal, isOpen: false });
  };
  //Definir funcion de exportacion
              ...
              {processor.failedList.length > 0 && (
                <div>
                  <p className="font-bold text-red-500">
                    Registros fallidos ({processor.failedList.length}):
                  </p>
                  <ul className="max-h-40 overflow-y-auto">
                    {processor.failedList.map((item, index) => (
                      <li key={`failed-${index}`} className="text-sm">
                        {item.name || item.nombre}{" "}
                        {item.last_name || item.apellido} -{" "}
                        {item.ci || item.cedula}: {item.error}
                      </li>
                    ))}
                  </ul>
                </div>
