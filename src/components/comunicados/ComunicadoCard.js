  ...
  return (
    <>
      <Card className={`h-full overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300 ${className}`}>
        <div className="p-4">
          <h2 className="text-xl font-bold text-[#0f2e5a] mb-2">{comunicado.titulo}</h2>
          <p className="text-gray-700 mb-4">{comunicado.descripcion}</p>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {comunicado.imagen1 && (
              <div 
                className="h-48 relative rounded-lg overflow-hidden cursor-pointer"
                onClick={() => openImageModal(comunicado.imagen1)}
              >
                <Image
                  src={comunicado.imagen1}
                  alt={comunicado.titulo || "Comunicado"}
                  fill
            ...
            {comunicado.imagen2 && (
              <div 
                className="h-48 relative rounded-lg overflow-hidden cursor-pointer"
                onClick={() => openImageModal(comunicado.imagen2)}
              >
                <Image
                  src={comunicado.imagen2}
                  alt={comunicado.titulo || "Portada"}
                  fill
                  style={{ objectFit: "cover" }}
                  className="hover:scale-105 transition-transform duration-300"
                />
