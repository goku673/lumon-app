...
const CompetitorRegister = ({ onSubmit, onBack, initialData = {}, guardians = [] }) => {
  ...
  const { data: areaLevelGrades, isLoading: isAreaLevelGradesLoading, isError: isAreaLevelGradesError, error: areaLevelGradesError } = useGetAreaLevelsGradesQuery();
  const { data: schools, isLoading: isSchoolsLoading, isError: isSchoolsError, error: schoolsError } = useGetSchoolsQuery();
  ...
  const { data: provinces, isLoading: isProvincesLoading, isError: isProvincesError, error: provincesError } = useGetProvincesQuery();

  const [debug, setDebug] = useState({
    gradesLoaded: false,
    areaLevelGradesLoaded: false,
    schoolsLoaded: false,
  ...
  useEffect(() => {
    setDebug(prev => ({
      ...prev,
      gradesLoaded: !!grades,
      areaLevelGradesLoaded: !!areaLevelGrades,
    ...
    if (isGradesError) console.error("Grades Error:", gradesError);
    if (isAreaLevelGradesError) console.error("Area Level Grades Error:", areaLevelGradesError);
    if (isSchoolsError) console.error("Schools Error:", schoolsError);
    if (isDepartmentsError) console.error("Departments Error:", departmentsError);
    if (isProvincesError) console.error("Provinces Error:", provincesError);
  }, [
    grades, areaLevelGrades, schools, departments, provinces,
    isGradesError, isAreaLevelGradesError, isSchoolsError, isDepartmentsError, isProvincesError,
    gradesError, areaLevelGradesError, schoolsError, departmentsError, provincesError
  ]);
